...
Leveraging Machine Learning for Strategic Advantage in March Madness Predictions (Use at your own risk in office pools)
...
> **Attention:** Since writing this article Kenpom.com has taken serious defensive measures against webscraping. 
> While the below code will no longer work its principals still apply to the data. Additionally with some creative thought,
> such as use of a browser table to csv extension, can be modified to produce the same output. That being said I cannot confirm 
> or deny the existence of a backup of the data so we can still iterate on our model
